# explore-ui-pwa Specification

## Purpose

The Explore UI is installable as a standalone app on mobile and desktop, with a service worker that makes the app shell load instantly and offline without ever serving stale query or telemetry data, and that updates itself without depending on the browser's own re-check to happen in time.
