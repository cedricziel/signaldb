...
TBD — the `mcp-server` capability (transport, auth, and the bulk of its
tool surface) is implemented and live but its own change has not archived
yet; this main spec currently holds only the `get_profile` requirement
synced from `profile-payload-access`. Update Purpose after the base
`mcp-server` change archives.
