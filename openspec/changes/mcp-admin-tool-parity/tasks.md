...
- [x] 6.2 Docs (routed via the docs skill): `docs/users/mcp.md` tool catalogue (platform-admin + tenant_* families, confirmation, annotations, the human-session-only boundary), `docs/users/authentication.md` (tenant self-service table with SDK/CLI/MCP columns), `docs/operations/table-provisioning.md` (SDK/CLI/MCP/UI paths), `docs/users/explore-ui.md` (new Tables section), `docs/users/querying-sql.md`/`querying-ir.md` (new query flags), `docs/users/schema-registry.md` (two new schema tools), `docs/architecture/openapi-codegen.md` (tenant.rs in the codegen story, `OPERATIONS` const, drift guard). No standalone CLI reference doc exists in this repo (checked); CLI usage is documented inline in the feature docs above, per existing convention. `docs/architecture/overview.md` was flagged by `check-doc-freshness.sh` (its `sources` glob is `endpoints/**`) but not edited — it documents the admin API table, not tenant.rs, and tenant.rs's change here is OpenAPI metadata only, no behavior change; a broad-glob false positive.
- [x] 6.3 Updated `multi-tenancy` skill (full Management API endpoint table + the `authorize_tenant` boundary + which CLI/MCP surfaces reach what, and why; tenant self-service table gained SDK operation ids) and `tempo-api` skill (pointer from the admin API table to the SDK/CLI/MCP surfaces, and to the management API). `dev-workflow` does not list CLI groups — no change needed.
- [x] 6.4 `cargo fmt --check` clean; `cargo clippy --workspace --all-targets --all-features -- -D warnings` clean; `cargo machete --with-metadata` — no unused deps; `pnpm lint`/`pnpm typecheck`/`pnpm vitest run` clean (6 pre-existing, unrelated locale-formatting test failures on this machine's non-en-US default `Intl` locale — `vizFormat.test.ts`, `MetricsChart.test.tsx`, `TraceFacets.test.tsx`, `TracesView.test.tsx`; not touched by this change, not new); `openspec validate mcp-admin-tool-parity --type change --strict` passes. Closing #627/#628 with the rescoping note is done via the PR body (this agent has no issue-closing access beyond the PR's "Closes" keywords).
