...
Every release build links eight binaries (`signaldb`, `signaldb-acceptor`, `-router`, `-writer`, `-querier`, `-compactor`, `-mcp`, `-cli`), and with the release profile's thin LTO each link re-optimises the whole DataFusion/Arrow/Iceberg graph: measured 432 s per binary on an 8-core machine versus 32 s without LTO. On the 4-vCPU CI runner that is the 25-minute tail of the musl image job (38 min warm, 47 min cold) and therefore the main-to-image latency for every deploy. The monolithic `signaldb` binary already links all five services; the per-service binaries are the same code with a different `main`. Shipping one server binary that takes the service to run as its first argument removes six of the eight LTO links while keeping LTO — no runtime performance trade.

## What Changes

- The `signaldb` binary becomes a multi-call binary: it runs the monolith when invoked as `signaldb`, and runs a single service when the first argument names one: `signaldb <service>` (`acceptor`, `router`, `writer`, `querier`, `compactor`, `mcp`). Selection is by argument only — the executable's file name is never consulted, so there is no argv[0]/symlink magic. Each service keeps its own clap surface (`signaldb acceptor --help` shows the acceptor's flags and `--version`).
...
- `service-binary-dispatch`: how the `signaldb` binary selects monolith vs single-service mode (first argument), what each mode's CLI surface is, and what container images and release artifacts ship.
