...
- Runtime: `common::datafusion_runtime::bounded_memory_pool` ⇒ `TrackConsumersPool<FairSpillPool>` (`flight.rs:445`, called from `session_context_with_limits`); `memory_limit_mb` default `None`, `max_concurrent_queries_per_tenant` default `None` (`src/common/src/config/mod.rs:1227`, `:1232`); admission semaphore exists but is inert (`flight.rs:605-620`).
