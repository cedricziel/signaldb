...
- [ ] 4.1 Failing test: OpenAPI document lists `/ui/session/oidc/start` and `/ui/session/oidc/callback` with schemas and an empty security requirement (`/ui/session/config` and its required-nullable `oidc: {name}` schema already exist from `dedicated-login-page`); the probe returns `oidc: {name}` with a configured, discovered provider, `oidc: null` while discovery is `unavailable`, and `password_enabled: false` under `disable_password_login`
