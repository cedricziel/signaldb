...
- [ ] 4.1 Failing test: OpenAPI document lists `/ui/session/oidc/start` and `/ui/session/oidc/callback` with schemas and an empty security requirement (`/ui/session/config` and its required-nullable `oidc: {name}` schema already exist from `dedicated-login-page` — done); the probe returns `oidc: {name}` with a configured, discovered provider, `oidc: null` while discovery is `unavailable`, and `password_enabled: false` under `disable_password_login` (still open — `login_config` is hardcoded to `oidc: None`/`password_enabled: true`; the failing test is `endpoints::oidc::tests::login_config_probe_reports_oidc_when_available`, `#[ignore]`d pending task 4.2)
- [ ] 4.2 Replace the probe's constant body with the provider state; add the two OIDC endpoints to `paths(...)`; regenerate `api/signaldb-api.json`, the Rust SDK, and the UI TypeScript client
