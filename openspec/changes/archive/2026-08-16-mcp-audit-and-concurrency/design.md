...
**D2 — Span factory + registry.** `common::self_monitoring::spans::mcp_tool_span(tool: &str, tenant_id: &str, dataset_id: Option<&str>, session_id: &str) -> Span`: INTERNAL, name `tools/call {tool}`, fields `mcp.method.name="tools/call"`, `gen_ai.tool.name`, `mcp.session.id`, `signaldb.tenant.id`, `signaldb.dataset.id`, `error.type` (empty, recorded on failure). Cover the MCP names in `otel/registry/signaldb.yaml`: `mcp.session.id` is referenced from the upstream (deprecated) definition; `mcp.method.name` and `gen_ai.tool.name` are annotated `dependency_resolution: exclude` upstream, so a `ref` fails resolution and a local redeclaration collides under `live-check --include-unreferenced` — they stay pinned by the semconv-crate constants and are validated by live-check through the upstream shells (revisit when the pin follows the GenAI registry). Both `registry check` modes pass. Status Error only for `outcome=error`.
...
**D4 — Trace continuity: the SDK injects W3C context on outbound requests.** Owned by `sdk-retry-on-throttle` (its `ClientHooks::exec` override injects `traceparent`/`tracestate` from `tracing::Span::current()` via the OTel propagator, feature `signaldb-sdk/tracing`, default on). This change only asserts the end-to-end result once that has landed; it does not touch the SDK.
...
**D6 — Concurrency: per-session `tokio::sync::Semaphore`.** The transport builds one `McpServer` per session (stdio has exactly one), so the semaphore lives on the handler instance and is dropped with the session — no `DashMap` on `McpAppState` and no eviction hook needed (the originally sketched map is equivalent but leaks without one). Default permits 8 (`[mcp].max_concurrent_tool_calls`), acquire with `timeout(2s)`; on timeout return `ErrorData::invalid_request("too many concurrent tool calls (limit 8); wait for in-flight calls to finish", data {limit})` and audit `outcome=error, error.type=concurrency_limit`. Stdio: single shared semaphore. _Alternative:_ a global cap — rejected, one session must not starve another.
