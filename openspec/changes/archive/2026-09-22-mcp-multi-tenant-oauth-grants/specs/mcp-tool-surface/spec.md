...
- **THEN** the tool rejects the call with an error naming both tenants,
  ...
  longer an implicit session-wide default to omit them in favor of; for a
  single-tenant credential the argument only confirms the caller's
  assumption against what this specific call authenticated as, it never
  selects which credential/tenant a call authenticates as
