...
- [x] 2.2 Lower `correlate` to a DataFusion hash join with the time-range filter on both scans, add `[querier].correlate_max_rows` (default 5,000,000) and the truncation warning. Verify `cargo test -p querier` passes. `correlate_max_rows` is threaded from config via `IrService::with_correlate_max_rows` (set from `QuerierConfig` at both Flight-service construction sites); `parent.`-scoped attribute-container references (`parent.span.<key>`, `parent.resource.<key>`) are lowered against the parent side the same way the child side lowers them. The truncation warning is ground truth from the querier, enforced _streaming_: `lower_correlate` wraps the join in `CorrelateCapExec` (`querier::query::correlate_cap`), a DataFusion extension node/physical operator that passes rows through unchanged up to the cap and flips a shared `Arc<AtomicBool>` — never buffering more than the one batch in flight, so a following `aggregate`/`where`/`limit` runs against a live stream rather than a fully materialized join. Whether the join was truncated is only known once the stream finishes, too late for the one Flight schema message sent first, so it rides as a trailing, data-free `FlightData` (`app_metadata` = `common::flight::CORRELATE_TRUNCATED_APP_METADATA`) the router reads while draining the stream, rather than either sniffing the document/row-count coincidence or Arrow schema metadata.
