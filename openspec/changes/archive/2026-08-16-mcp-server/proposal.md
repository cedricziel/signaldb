...
- **Read/query tools** (available to every authenticated tenant session, no role gating in v1): `search_traces`, `get_trace`, `search_logs`, `query_metrics`, `discover_attributes` (signal-aware: `traces` via Tempo tags, `logs` via Loki labels, `metrics` via Prometheus labels), `discover_metrics` (tenant metric names), plus discovery tools `list_datasets` / `list_schemas` / `list_tables`.
