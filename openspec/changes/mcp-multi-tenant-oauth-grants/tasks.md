...
- [x] 5.6 Change `sdk_client_for`/`router_client` to set `X-Tenant-ID`/`X-Dataset-ID` explicitly from the tool call's own arguments when the credential is OAuth with a multi-tenant grant. Wired for every tenant-scoped tool (~25 query/discovery tools plus 14 tenant self-management tools) except `tenant_get_schema`, deliberately left unreachable by a multi-tenant credential — see design.md's Non-Goals for why (fails closed, not a security gap)
