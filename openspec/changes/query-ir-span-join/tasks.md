...
- [x] 2.2 Lower `correlate` to a DataFusion hash join with the time-range filter on both scans, add `[querier].correlate_max_rows` (default 5,000,000). Verify `cargo test -p querier` passes. **Partial**: the row cap is applied as a hardcoded limit matching the config default; the config value isn't threaded into the planner yet, and the truncation warning (task's row-cap scenario) isn't implemented — both left as follow-up.
- [ ] 2.3 Add a `tests-integration` test that ingests OTLP traces for two tenants across three services and checks caller/callee pair counts and tenant isolation through `POST /api/v1/query`. Verify it passes. **Not done** — left for a follow-up PR.
