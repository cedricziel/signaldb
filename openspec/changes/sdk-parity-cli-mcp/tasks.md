...
- [ ] 2.1 Write failing test asserting the CLI crate contains no direct
      `FlightServiceClient`/raw-HTTP construction against a SignalDB service
      (`cargo test -p signaldb-cli`)
- [ ] 2.2 Write failing tests for the single `query` command: a required,
      mutually-exclusive language flag (`--sql`/`--promql`/`--logql`/
      `--traceql`); missing/duplicate flag → usage error, non-zero exit
- [ ] 2.3 Write failing tests for native per-language output: `--sql` gives
      `table`/`csv`/`ndjson` rows; `--promql`/`--logql`/`--traceql` pass through
      the native Prometheus/Loki/Tempo JSON
- [ ] 2.4 Write failing tests for deterministic exit codes (success `0`,
      empty-result `0`, error non-zero with diagnostics on stderr)
- [ ] 2.5 Implement `query --<lang>` dispatch through the SDK; replace
      `commands/query.rs` Flight usage with SDK calls
- [ ] 2.6 Replace `tui/client/flight.rs` Flight usage with SDK query calls
- [ ] 2.7 Re-taxonomize the command tree: move top-level `tenant`/`api-key`/
      `dataset` under `admin`; keep `tui`/`completions`/user bootstrap
- [ ] 2.8 Write failing test for endpoint/credential resolution precedence
      (flag > env > config); implement resolution

## 3. Phase 1 — MCP query parity

- [ ] 3.1 Write failing tests for MCP query tools (one per language) returning the
      SDK's native shapes (`cargo test -p mcp-server`)
- [ ] 3.2 Implement the MCP query tools over the SDK query methods
- [ ] 3.3 Write failing test that the same query in the same language via MCP and
      via the CLI yield equivalent data (`cargo test -p tests-integration`)

## 4. Phase 1 — Parity enforcement

- [ ] 4.1 Write the failing three-way parity test in `tests-integration`:
      enumerate the SDK public capability surface, assert each has a CLI
      verb/flag and an MCP tool (covering query + admin)
- [ ] 4.2 Make the parity test pass; wire it into CI

## 5. Phase 2 — Operational control API (router proxy)

- [ ] 5.1 Write failing router tests for `/api/v1/ops/*`: trigger compaction,
      read status, retention/snapshot/orphan actions incl. dry-run, and
      admin-authorization rejection (`cargo test -p router`)
- [ ] 5.2 Implement the `/api/v1/ops/*` handlers forwarding to the compactor
      control surface (implement the router Flight `do_action` path or HTTP
      equivalent)
- [ ] 5.3 Annotate the ops endpoints in the code-first OpenAPI surface
- [ ] 5.4 Run `cargo xtask generate`; verify ops methods appear in `signaldb-sdk`
      and the TypeScript client in `src/ui/src/api/gen`
- [ ] 5.5 Add compactor-side integration coverage for proxied actions and
      dry-run semantics (`cargo test -p tests-integration`)

## 6. Phase 2 — CLI + MCP ops parity

- [ ] 6.1 Write failing tests for CLI `ops <verb>` verbs over the SDK
      (`cargo test -p signaldb-cli`)
- [ ] 6.2 Implement CLI `ops` verbs
- [ ] 6.3 Write failing test for the MCP ops tool(s) (`cargo test -p mcp-server`)
- [ ] 6.4 Implement MCP ops tool(s)
- [ ] 6.5 Extend the parity test (4.1) to cover ops; keep CI green

## 7. Docs & skills

- [ ] 7.1 Update `docs/architecture/overview.md` and `docs/users/
authentication.md` for the new CLI taxonomy and `query`/`ops` verbs
      (route via the docs skill)
- [ ] 7.2 Update the `multi-tenancy` skill's CLI references to the `admin ...`
      command form
- [ ] 7.3 Add a CLI changelog entry noting the BREAKING top-level → `admin` move

## 8. Validation

- [ ] 8.1 `cargo fmt`, `cargo clippy --workspace --all-targets --all-features`,
      `cargo machete --with-metadata`
- [ ] 8.2 `openspec validate sdk-parity-cli-mcp --strict`
