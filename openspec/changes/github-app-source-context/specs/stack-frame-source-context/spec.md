...
Cache entries SHALL be keyed by installation, repository, ref, and file, so repeated lookups for the same file within a bounded cache period SHALL be served without a new GitHub API call regardless of line. Line bounds SHALL be evaluated against the cached content on every request, so "line out of range" is never cached. An "unavailable" outcome caused by the content itself (missing, not a file, too large, undecodable) SHALL be cached the same way, while a transport or internal failure SHALL NOT be. Removing an installation link SHALL evict its cached entries immediately. The system, so that repeatedly viewing the same trace or profile does not consume GitHub API rate limit budget proportional to view count. The cache SHALL also enforce a fixed capacity and evict least-recently-used entries when that capacity is exceeded, independent of the cache period, so that a deployment fanning out across many distinct repo/ref/file combinations cannot grow the cache unbounded.
