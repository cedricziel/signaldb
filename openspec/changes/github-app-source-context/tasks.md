> First delivery (installation layer) shipped: sections 1, 2.1–2.3/2.8, 3, 4, 5, 7.3, 8.2–8.3, 9. The source-snippet lookup (2.4–2.7, 6, 7.1–7.2, 8.1) is the next step.
...
- [ ] 3.5 Write a failing test asserting removing an installation link causes an immediate-subsequent source-context fetch (Task 6) for that installation's repos to report "unavailable"; implement removal semantics to pass (`cargo test -p router`) — removal itself (row deleted, cached token dropped) shipped with 3.2; the source-context half waits on Task 6
