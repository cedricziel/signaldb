...
- [ ] 1.3 Define the `Comparison` response types in `common` (`utoipa::ToSchema` + serde): cohort counts, sampling scope (`none`/`measures`/`records`) and sample sizes, `statistic`, ordered `fields` (dimension / measure / nominal variants with participation, score, values or buckets, `truncated`, `sampled`), nullable `riskRatio`, nullable measure summaries, `skipped` list with reasons; unit tests: JSON round-trip and a serializer guard that rejects any non-finite float
- [ ] 1.4 Pure ranking helpers in `common`: base-2 Jensen–Shannon divergence, participation JSD, `score` per design D5, sort key `(p_sel > 0, !nominal, score desc, name asc)`, risk ratio / support per value — property tests: JSD ∈ [0,1], identical distributions → 0, disjoint → 1, determinism, empty-cohort inputs yield score 0 and finite outputs, selection-only value yields `riskRatio: null`

## 2. Planner execution (querier)

- [ ] 2.1 Failing planner tests over an in-memory traces fixture (`cargo test -p querier`): heatmap-box selection yields the expected cohort counts; group-vs-rest selection; absent → baseline; empty selection and all-selected succeed with zero shares, null summaries, score 0 and a finite payload; both tenants isolated
...
- [ ] 2.3 Implement `CompareAccumulator` fold: per-row cohort flag, promoted-column values, map `(key, value)` entries attributed to their container with promoted keys skipped in the map walk (design D1 de-duplication; fixtures for equal and conflicting values); participation and value counts per cohort; field cap with `skipped: field-cap`; nominal degradation past the D4 threshold; legacy JSON-string attribute columns decoded via the existing reconciliation
...
- [ ] 2.5 Failing tests then implementation for measures (D6): order-independent identity-hash threshold reservoir (bottom-k) per cohort with the `[1000, 65536]` bound, quantile bucket edges (≤16, deduped, 1-2-5 snap for durations), shared axis across cohorts, min/max/median (null on empty cohort), `sampled` flag; determinism tests: two runs byte-identical **and** the same data fed as reordered/re-partitioned batches yields an identical payload
...
- [ ] 2.7 Serialize `Comparison` into a single-row `comparison_json` batch and route it through the IR do_get path like `flamegraph`; add the matched-row hard cap with the identity-hash record sampling fallback (scope `records`, precedence over `sample`) and a test that the response states scope and counts
- [ ] 2.9 Source coverage tests: `profiles` compare over metadata rows with `samples_json`/`stacktraces_json` skipped as retrieval-only; `metrics` compare over gauge+sum points including a JSON-string legacy attribute fixture
...
- [ ] 5.4 Group-row entry point: "compare to the rest" action on grouped traces and logs tables emitting `field = value`; test that the built document carries filters/range/extract plus `compare` only — no `aggregate`/`topk`/`order`/`limit` from the grouped view
- [ ] 5.5 Refinement actions: "only this value" / "exclude this value" (`or(not(exists), field != v)`, test that absent-field records survive) / "group by this field" on dimension bars, "below" / "above" on measure buckets, dispatching to the existing filter and grouping state; tests that a filter becomes visible and removable and that grouping switches view
