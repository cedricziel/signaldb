...
the terminal `compare` stage that partitions matched records into a selection
and a baseline cohort by predicate and contrasts their value distributions
across a field set. An unknown stage, or a stage illegal for the source or IR
version, SHALL be rejected as unsupported rather than silently ignored. The
`extract` stage SHALL support the `json` and `logfmt` parsers; the `regex`
parser is not part of this capability and, together with the predicate `regex`
operator, SHALL run only behind a bounded, timeout-guarded matcher.
...
#### Scenario: A v4-only compare is rejected under earlier versions

- **WHEN** a client submits a `compare` stage or `comparison` result envelope
  with `irVersion` 1, 2, or 3
...
IR v1; `heatmap` additionally in IR v2; `comparison` additionally in IR v4;
and, for the `profiles` source only, `flamegraph`), and the system SHALL
validate the declared envelope against the inferred terminal relation type and
against the selected source, rejecting a mismatch before execution. Each
...
contract. The columns of a `rows`/`table` result SHALL be a curated
projection: taken from an explicit document-level `fields` list of logical
names when present, otherwise a bounded server default — never all physical
columns implicitly. A `fields` entry absent from the terminal relation, or a
`fields` list on a `series`, `heatmap`, `comparison`, or `flamegraph` result,
SHALL be rejected.
...
- **WHEN** a query's `fields` list names something the terminal relation
  does not carry, or a `series`, `heatmap`, `comparison`, or `flamegraph`
  query declares `fields`
...
#### Scenario: Comparison envelope requires a terminal compare

- **WHEN** a query declares the `comparison` envelope but its pipeline does not
  end in a `compare` stage
- **THEN** the query is rejected at validation as an envelope mismatch
