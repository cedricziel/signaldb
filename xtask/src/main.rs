...
fn generate_sdk_client(spec: &serde_json::Value) -> Result<String> {
    ...
    // (design D3) — deriving it here, in the same run that emits the
    // generated client methods, keeps it from drifting out of sync with what
    // progenitor actually generated. Extracted from the original (still
    // 3.1-shaped) spec, before it is cloned, downconverted, and moved into
    // `openapiv3::OpenAPI` below — operationId strings are untouched by that
    ...
    settings
        ...
        // The retry policy rides along as progenitor's "inner" value
        // (`Client::new(baseurl, policy)` / `new_with_client(.., policy)`;
        // reachable via `ClientInfo::inner()`), and the hand-written
        // `impl ClientHooks<RetryPolicy> for Client` in
        // `src/signaldb-sdk/src/retry.rs` overrides `exec` for every
        // generated operation through progenitor's documented auto-ref
    ...
    formatted.push('\n');
    formatted.push_str(&operations_const);

    Ok(format!("{SDK_GENERATED_HEADER}\n{formatted}"))
}

/// Header written above the generated SDK client.
const SDK_GENERATED_HEADER: &str = "\
...
/// Walks `spec["paths"]`, collecting the `operationId` of every HTTP-method
/// operation object under every path.
fn extract_operation_ids(spec: &serde_json::Value) -> Vec<String> {
    const METHODS: &[&str] = &[
    ...
    if let Some(paths) = spec.get("paths").and_then(|p| p.as_object()) {
        for path_item in paths.values() {
            ...
            for method in METHODS {
                if let Some(operation_id) = path_item
                    .get(*method)
