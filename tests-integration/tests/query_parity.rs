...
const EXCLUDED: &[(&str, &str)] = &[
    ...
    (
        ...
        "requires a signed-in human instance-administrator session (`TenantContext::is_instance_admin`, set only by password login); unreachable from any API-key-authenticated client, which is the only auth the CLI and MCP server support",
    ),
    (
        "list_tenants_self",
...
const MANIFEST: &[(&str, CliSurface, &str)] = &[
    ...
    // ---- Tenant self-management (management API + tenant tables/schemas) ----
    (
