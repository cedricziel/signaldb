...
# Create signaldb.toml for local development
...
if [ "$MODE" = "monolithic" ]; then
    ...
    cargo run --bin signaldb > "${LOG_DIR}/monolithic.log" 2>&1 &
    PIDS="$!"
    ...
    cargo run --bin signaldb-writer -- --flight-port 50051 > "${LOG_DIR}/writer.log" 2>&1 &
    PIDS="$PIDS $!"
