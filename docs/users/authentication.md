...
| POST   | `/api/v1/tenants/{tenant_id}/source-context` | Source snippet around a stack-frame location via the tenant's linked GitHub App installation(s); `200` with `status: available\|unavailable` unless the request is malformed or unauthorized (codes in [the GitHub App guide](../operations/github-app.md#source-context)) | `source_context`         | `signaldb-cli tenant source-context` / `get_source_context`                     |
