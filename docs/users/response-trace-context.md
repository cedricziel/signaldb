...
file defines — those are the `signaldb.*` metrics (WAL — including the
`signaldb.wal.instance_cap_hits` cap counter — Flight and RPC counters,
histograms and gauges) exported over OTLP, not part of the response headers
described here.
