...
- **Discovery reads**: `signaldb.discovery` INTERNAL span around
  `describe: fields` / `describe: values` and `GET /api/v1/query/sources`,
  with `signaldb.discovery.kind` (`fields`/`values`/`sources`),
  `signaldb.discovery.source`, and `signaldb.discovery.cost_mode` — the
  metadata path's cost, next to the query stages, though it never reaches a
  querier.
