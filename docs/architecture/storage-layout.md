...
- **Straight to current, additions only, when the starting version is unknown**: a table with no recorded property (pre-dates this mechanism) skips hop-walking and migrates directly to the current version in one step, never removing a field. Hop-by-hop removal assumes the starting shape is trusted; an inferred baseline isn't — a table already ahead of an early hop could otherwise lose fields the diff can't tell it legitimately has.
