...
- Executes one partition per job: the rewrite reads only that partition (bounded by `[compactor] memory_limit_mb`, spilling past it) and commits a *delta* — the input files are removed and the compacted outputs added in one snapshot, leaving every other partition referenced unchanged. Concurrent ingest therefore does not invalidate the commit; only a change to the job's own input files does
...
- The four lifecycle cycles — compaction, lease expiry, retention, orphan cleanup — each run on their own task at their own cadence, so none can postpone another. Retention and compaction may therefore commit against one table concurrently (Iceberg optimistic concurrency retries on conflict), while retention never overlaps itself: the orphan-cleanup pass pre-expires snapshots through the same enforcer, and both paths are serialized. Every iteration is also guarded against panics — caught, counted, retried with backoff — instead of ending its cycle's task permanently; `/health` reports `503` while a cycle is in that backoff
