...
- The four lifecycle cycles — compaction, lease expiry, retention, orphan cleanup — each run on their own task at their own cadence, so none can postpone another. Retention and compaction may therefore commit against one table concurrently (Iceberg optimistic concurrency retries on conflict), while retention never overlaps itself: the orphan-cleanup pass pre-expires snapshots through the same enforcer, and both paths are serialized
