...
- Executes one partition per job: the rewrite reads only that partition (bounded by `[compactor] memory_limit_mb`, spilling past it) and commits a _delta_ — the input files are removed and the compacted outputs added in one snapshot, leaving every other partition referenced unchanged. Concurrent ingest therefore does not invalidate the commit; only a change to the job's own input files does
