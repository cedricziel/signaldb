...
//! stream: zero messages, not even a schema frame; that decodes to "no
//! rows" rather than a decode failure. Decoding otherwise goes through
//! [`common::flight::decode::flight_data_vec_to_batches`], which (unlike
//! `arrow_flight::utils::flight_data_to_batches`) tracks dictionary
//! batches, so a querier response containing dictionary-encoded columns
//! decodes correctly instead of erroring (#951).
...
pub(crate) async fn decode_flight_batches(
    data: Vec<FlightData>,
    what: &str,
    ...
    flight_data_vec_to_batches(data).await.map_err(|e| {
...
mod tests {
    ...
    #[tokio::test]
    async fn empty_stream_decodes_to_no_batches() {
        let batches = decode_flight_batches(vec![], "logs")
            .await
    ...
    #[tokio::test]
    async fn frames_decode_to_batches() {
        let (schema, batch) = sample_batch();
        let data = batches_to_flight_data(&schema, vec![batch]).expect("encode");
        let batches = decode_flight_batches(data, "logs").await.expect("decode");
        assert_eq!(batches.len(), 1);
        assert_eq!(batches[0].num_rows(), 2);
    }

    #[tokio::test]
    async fn garbage_frames_are_a_500() {
        let bogus = FlightData {
            data_body: vec![1, 2, 3].into(),
            ..Default::default()
        };
        let err = decode_flight_batches(vec![bogus], "logs")
            .await
