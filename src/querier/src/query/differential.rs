...
//! | `sum by (StatusCode) (count_over_time(...))` (mixed-case attribute grouping) | **(1) old path was wrong, fixed (issue #1392)** | #1070's fix (`ident()` not `col()` for a group-column alias) only touched `ir_planner.rs`. The *old* LogQL metric path (`logs.rs`'s `execute_plan`) had the identical unfixed bug for grouping by a mixed-case attribute label, and stayed reachable after §5.3's switch removal — both as the D5 fallback and as each operand of a vector-to-vector binary metric query (`LogsService::query_metric` routes both operands of any `MetricQuery::Binary` `classify_binary` accepts straight to `execute_plan`), so e.g. `sum by (StatusCode)(count_over_time(a[1m])) / sum by (StatusCode)(count_over_time(b[1m]))` reproduced it. Fixed in `execute_plan`'s grouping/projection column resolution the same way (`ident()` instead of `col()`), pinned in `logs.rs`'s `execute_plan_groups_by_mixed_case_materialized_label`; `mixed_case_grouping_label_groups_correctly` still pins the IR path's own grouping. |
...
//! | `sum(count_over_time(...))` (an **explicit** vector aggregation with no `by()`) | **(1) `ql_ir` was wrong, fixed (CodeRabbit review on #1393); old path also wrong, fixed (#1394)** | D7's stream-identity default was first applied too broadly: `lower_metric_query` treated *any* empty grouping — including `vector_grouping`'s `Ok(Vec::new())` for an explicit `v.grouping == None` — as "apply the default", when an explicit ungrouped vector aggregation means the opposite (collapse to one series, same as an empty `by` anywhere else). Fixed by applying `STREAM_IDENTITY` only in the bare-`MetricQuery::Range` arm, pinned in `ql-ir`'s `explicitly_ungrouped_vector_aggregation_collapses_to_one_series`. Checking that fix against the old path (not assumed) surfaced a *second*, independent finding: `logs.rs::execute_plan` never collapsed an ungrouped `sum(...)` either — `sum`'s `outer_agg` stays `None` ("folds into the grouped range aggregate"), so the old path's grouping branch treated it identically to a bare range aggregation (`SERIES_COLUMNS`, no reduction pass), producing one row per stream instead of one row total. This was the old path's *own* bug, reachable through the binary-vector-operand and D5-fallback routes even after §5.3's switch removal made the single-query top-level path IR-first — fixed via a new `MetricPlan::outer_sum` flag that lets `execute_plan` tell an explicit ungrouped `sum` apart from a bare range aggregation (#1394, sibling to the still-open #1392). Pinned directly against `execute_plan` in `logs.rs`'s `sum_with_no_by_collapses_to_one_series` (a differential-style fixture is unnecessary here: the bug was in the old path alone). `explicitly_ungrouped_vector_aggregation_collapses_to_one_row` still pins the IR path's own collapse. |
...
/// `logql_metric.rs::plan_metric_query` documented that `sum`'s `outer_agg`
/// stayed `None` ("folds into the grouped range aggregate"), so
/// `logs.rs::execute_plan`'s grouping branch (`None if
/// out_group_cols.is_empty() => SERIES_COLUMNS`) treated an ungrouped
/// `sum(...)` identically to a *bare* range aggregation — same
/// `SERIES_COLUMNS` grouping, no second reduction pass — producing one row
/// per stream instead of one row total. Fixed as #1394 (a
/// `MetricPlan::outer_sum` flag distinguishes the two cases); see
/// `logs.rs`'s `sum_with_no_by_collapses_to_one_series` for the direct pin
/// on `execute_plan`. This test itself is a plain regression pin on the IR
/// path's own (always-correct) collapse.
#[tokio::test]
