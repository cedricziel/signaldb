...
use crate::schema::typed_attributes::{home_column, residue_column, typed_columns};
...
pub fn is_typed_layout(schema: &Schema, container: &str) -> bool {
    schema.field_with_name(&residue_column(container)).is_ok()
}
...
pub fn compat_attr_expr(schema: Option<&Schema>, container: &str, key: &str) -> Expr {
    ...
    let home = |canonical: CanonicalType| get_field(col(home_column(container, canonical)), key);
    coalesce(vec![
