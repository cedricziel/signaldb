...
/// per column. Column names are resolved through
/// [`crate::iceberg::evolution::resolve_label_columns_fresh`] -- the same
/// mechanism [`crate::schema_parser::ResolvedSchema::build_iceberg_schema`]
/// uses to create the columns themselves -- so two keys that sanitize to the
/// same candidate name each get their own bloom-filter property instead of
/// the second key silently going unfiltered (#1448). An exact-duplicate key
/// still collapses to a single property.
///
...
pub fn bloom_filter_properties_for_labels(labels: &[String]) -> Vec<(String, String)> {
    ...
    crate::iceberg::evolution::resolve_label_columns_fresh(labels)
        .into_iter()
        .map(|(_, column)| {
            (
                format!("{WRITE_PARQUET_BLOOM_FILTER_ENABLED_COLUMN_PREFIX}{column}"),
...
mod tests {
    ...
    fn bloom_filter_properties_for_labels_gives_colliding_keys_distinct_properties() {
        ...
        let properties = bloom_filter_properties_for_labels(&labels);
        assert_eq!(
            properties,
            vec![
                (
