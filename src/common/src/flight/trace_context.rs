...
/// the low-cardinality ticket verb; `server_address` is `host:port` of the
/// target Flight service (from discovery metadata). Callers instrument the
/// call future with the returned span and record its outcome via
