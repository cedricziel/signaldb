...
//
// One post-processing step is applied to progenitor's output: the empty
// `impl ClientHooks<crate::retry::RetryPolicy> for &Client {}` progenitor emits
// is replaced with an `exec` override that routes every operation through
// `crate::retry::execute` (the shared retry-on-throttle policy). See
// `install_retry_exec_hook` in `xtask/src/main.rs`.
...
impl ClientHooks<crate::retry::RetryPolicy> for &Client {
    async fn exec(
        ...
        crate::retry::execute(self.client(), self.inner(), request, info).await
    }
}
impl Client {
    /**List all tenants
