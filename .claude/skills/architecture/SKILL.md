...
7. Semantics of attribute keys / entity types / metric names come from the schema registry (change `schema-registry`): the vendored OTel semconv (`vendor/otel-semconv/`) and `otel/registry/` are parsed by the `schema-model` crate (parser + resolver + validator, no FDAP deps) into bundled read-only registries; tenant custom registries in the same Weaver model are stored in the catalog.
