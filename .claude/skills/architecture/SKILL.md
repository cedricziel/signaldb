...
6. Router also nests query APIs for every signal, each lowering a parsed query to a DataFusion `Expr`/aggregate over the signal's Iceberg tables (never a SQL string, matching the trace path):
   ...
   - `POST /api/v1/query` (native **Query IR**, epic query-ir-core): the first-party structured query surface (versioned JSON IR over `logs`/`traces`/profile-summary `profiles`/scalar `metrics`, not a dialect string). `endpoints/query.rs` selects the source's read scope before it stamps the server clock and forwards a `query_ir:{tenant}:{dataset}:{json}` ticket to the querier's `IrService`, which validates + lowers the IR to a DataFusion `DataFrame` (promotion-invariant field resolution, three-valued absent semantics) and returns `rows`/`series`/`table` envelopes. Profile rows expose only registered scalar metadata (`profile.id`, timestamps/duration, sample/period metadata, service and trace/span IDs) plus logical profile/scope/resource attributes; never `samples_json`, `stacktraces_json`, or raw attribute payloads. `metrics` unions `metrics_gauge`+`metrics_sum` (one scalar `metric.value` per row, `metric.name`/`service.name` plus resource attributes); `metrics_histogram` and PromQL-only operations (`rate`/`irate`/`increase`/`histogram_quantile`/cross-series formulas) have no IR equivalent yet — `SourcePlan` scans a list of physical tables per source (`tables`, not a single `table`) and unions them onto `row_defaults` when there's more than one, which is what lets a source span multiple physical tables at all. Pyroscope endpoints retain flamegraph, diff, extraction, and heatmap work. IR v2 additionally has a terminal `heatmap` relation for sparse, epoch-aligned time-by-numeric distributions; trace duration heatmaps retain the precise span-start predicate and timestamp partition-pruning bounds. No trace-specific HTTP endpoint or Flight ticket exists. `endpoints/query.rs` encodes attribute containers as JSON objects (`map<string,string>`); `canonical_arrow_type` returns `Option` so "no canonical form" is distinct from "cast failed". This is the only surface that keeps OTel's three attribute scopes distinct: `SourcePlan` lists `log`/`span`/`profile`, `scope` and `resource` containers, a field may carry a container qualifier (`resource.`/`scope.`/`log.`/`span.`/`profile.`) to read exactly one where a bare name coalesces, and the logs `rows` default projects the whole LogRecord (trace context incl. `trace_flags`, `severity_number`, scope identity, schema URLs, all three containers unmerged). Physical addressing stays rejected through the resolver's `is_physical_name` check (the static `STORAGE_DENYLIST` was replaced by the logical schema's physical-name set), and retrieval-only fields such as the log `body` are rejected in predicates, grouping, and ordering — the widened defaults are server-chosen, not client-nameable. Client-visible built-ins (record metadata, join keys, standard resource/scope attributes) are declared in `LogicalSchema::core()` (`src/common/src/schema/logical.rs`), independent of the physical Iceberg layout. The UI (Explore "Query" tab) and CLI (`query --ir`) consume it via generated clients. See `tempo-api` skill and `docs/users/querying-ir.md`.
