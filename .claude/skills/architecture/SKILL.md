...
- Retry classification per failed job attempt (`compactor::executor`, #1052): commit conflicts and transient infrastructure failures (typed `object_store`/`io` errors plus a marker list for the stringly-typed catalog and iceberg-fork errors) share one exponential-backoff budget; anything not positively identified as transient is terminal and fails on the first attempt instead of repeating the rewrite
- Retention enforcement, snapshot expiration, and orphan file cleanup. Orphan liveness comes from the union of the retained snapshots' manifests (never snapshot age), and a re-validation pass rebuilds that set before every real deletion batch, unconditionally. Per-file deletion lines are `DEBUG` (a backlog pass deletes tens of thousands of files at boot); batch/run summaries are `INFO`
