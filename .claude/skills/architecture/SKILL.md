...
4. Results stream back as Arrow RecordBatches via Flight (trace not found -> Flight `not_found` status). HTTP responses also return the server span's trace context and stage timings (`Server-Timing`/`traceresponse`; see `docs/users/response-trace-context.md`)
