...
| `GET /tempo/api/search`                          | Implemented         | Trace search with filters -> routes to Querier; `spss` caps spans per span set in the response. `q` is parsed by the standalone `traceql` crate (syntax only). Lowering defaults to `querier/query/search_filter.rs`; `[querier].trace_search_via_ir` (temporary rollout switch, `ir-single-lowering`) routes it through `ql_ir::traceql_to_ir` + the shared IR planner instead, with `tags` lowered by `querier/query/tags_to_ir.rs`. Rejection class decides the status: input that is not TraceQL (`notbraces`, `{ foo }`, `{ zzz = 1 }`) is **400**; valid TraceQL we do not lower (`\|\|`, `!=`, `=~`, `duration`) is **501**. Nothing outside the subset is silently dropped |
