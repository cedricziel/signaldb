...
| Metrics  | `metrics_gauge`, `metrics_sum`, `metrics_histogram`, `metrics_exponential_histogram`, `metrics_summary` | `schemas.toml` (v2, inherits v1)                         |
| Profiles | `profiles`                                                                                              | `schemas.toml` (v2, inherits v1)                         |
...
`[schema.materialized_labels]` (per signal) promotes attribute keys from the `*_attributes` JSON into nullable `label_<key>` columns at ingest, so they match exactly / by regex / with ordered comparisons instead of the JSON substring approximation. Naming via `common::schema::materialized_column_name` (non-alphanumeric → `_`, `label_` prefix), collision-proof: two configured keys sanitizing to the same candidate get distinct columns via `common::iceberg::evolution::resolve_label_columns_canonical`/`resolve_label_columns_fresh` — the same doc-stamping `resolve_label_columns` the evolution path below uses, wrapped to sort the configured keys alphabetically before resolving, so reordering `[schema.materialized_labels]` (a no-op edit) can never reassign an already-colliding key to a different column. `ResolvedSchema::build_iceberg_schema` (schema creation) seeds resolution from the table's own base fields (so a label colliding with a base column is suffixed, not dropped) and stamps each label's `doc`; the writer's `label_columns_from_maps`/`materialized_label_columns_from_json` use the schema-less `resolve_label_columns_fresh` since the Flight ingest path resolves before any WAL/catalog round trip (#1448) — this agrees with table creation as long as the configured key _set_ hasn't changed since the table was created. When it has (a key added/removed on a table that already exists), the fresh and committed assignments diverge concretely -- e.g. table created with `logs = ["http_method"]` (→ `label_http_method`, doc-tagged); config becomes `["http.method", "http_method"]`; the Flight path now assigns `http.method` (sorts first) to `label_http_method` and `http_method` to `label_http_method_2`, backwards from the table's real schema -- which, uncorrected, would have `coerce_batch_to_schema`'s match-by-name silently write `http.method`'s values into `http_method`'s real column and drop `http_method`'s own values outright. `IcebergTableWriter::reconcile_label_columns` (called from `append_batches_with_marker`, right before `coerce_batch_to_schema`) fixes this up: for each configured key it compares the fresh column against `column_for_key(self.table.current_schema()?, key)` and renames the batch column when they differ, computing every rename from the batch's original schema and applying them in one pass so a two-key swap resolves correctly; a key with no promoted column yet whose fresh name collides with a column belonging to a different key has its batch column dropped (falls back to the JSON path) rather than overwriting that key's column. Promoting a genuinely new key to its own column still needs schema evolution (`add_label_columns`), which nothing triggers automatically for this static-config path (same gap as below) -- reconciliation makes a set change safe, not a substitute for promotion. Batch columns are matched by the origin key the writer stamps into each label column's Arrow field metadata (`LABEL_ORIGIN_KEY_METADATA`, survives the WAL's IPC round trip), not by name; a pre-#1534 WAL entry without that metadata falls back to the name-based guard (#1534). Writer populates from resource→scope→record attributes (first non-null); `coerce_batch_to_schema` drops columns a table lacks and null-fills nullable ones it has. New tables carry the configured columns from creation; existing tables can gain `label_<key>` columns post-creation via `common::iceberg::evolution::add_label_columns` (metadata-only `AddSchema`+`SetCurrentSchema` commit via `Catalog::update_table`; idempotent; collision-proof — two keys sanitizing to the same candidate name get distinct columns via a deterministic suffix, resolved by each column's origin-key `doc`, the authoritative key→column record, not by re-deriving the candidate name; field ids continue past nested map/list ids; no Parquet rewrite — old files null-fill on read, snapshot-pinned schemas stay reachable; needs iceberg-rust rev >= 96f28c18). Querier reads of a promoted-but-possibly-unbackfilled column always `COALESCE` with the attribute-map fallback rather than trusting the column alone, so results stay correct through the null-fill window (#816) — at the cost of the exclusive-column fast path for that column's lifetime. Tables not yet evolved fall back to JSON. Allowlists resolve per tenant (tenant schema override replaces global) at table creation (`ensure_table`) and in the writer transforms. Querier routes via the column when the table schema has it (LogQL `attribute_expr`, trace search `Condition::to_expr`, PromQL `matcher_expr`) — **known gap**: every resolution point recomputes `materialized_column_name(key)` directly rather than consulting `doc`/`column_for_key`, so under a collision (static or auto-promoted) a query for the second key still silently resolves to the first key's column instead of failing to resolve; interim guard (#1533): when a `<base>_<n>` sibling exists, resolvers skip the column fast path and fall back to attribute-map extraction for that key, so the result is correct but unaccelerated; the full doc-based resolution is still open. Implemented for all four signals: **logs** (exact/regex/ordered), **traces** (exact), **metrics** (exact/regex filter; `by`/`without` grouping; part of the natural series identity — PromQL scan projects the label columns, null-filled across the gauge/sum union), **profiles** (columns populated; no attribute-filter query surface yet). Metrics/profiles table schemas now resolve from `schemas.toml` the same way traces/logs do (`ResolvedSchema::to_iceberg_schema_with_labels` appends label columns for all eight built-in table types uniformly); metrics transforms extract per-exploded-data-point label values via `materialized_label_columns_from_json`. See `docs/architecture/storage-layout.md#materialized-labels`.

### Schema evolution (all eight signal tables)

`common::iceberg::evolution` covers every `schemas.toml`-sourced signal table (traces, logs, all five metrics representations, and profiles) via `TableManager::ensure_schema_evolved`: an existing table's schema is brought forward to the current `schemas.toml` version on every `ensure_table` load, not just at creation. `common::iceberg::evolution::ensure_schema_current` reads the table's `signaldb.schema.version` property, walks `SchemaDefinitions::version_chain` to the current version, and applies each hop via `apply_schema_migration` (additive `AddSchema`+`SetCurrentSchema`+`SetProperties` through `Catalog::update_table`, same commit/verify shape as `add_label_columns` below). A table with **no recorded version** (pre-dates this mechanism) skips hop-by-hop walking and migrates directly to current in one step, additions only, never removing a field — hop-by-hop removal assumes the starting version is trusted, which an inferred baseline isn't (a table already ahead of an early hop would otherwise lose fields the diff can't tell it legitimately has). No backfill of historical row values; `field_removals` (schemas.toml) only ever drops a column going forward. Field-created-fresh tables get their version stamped at creation, not left to the evolution path to catch up on first load.
