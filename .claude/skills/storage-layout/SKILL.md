...
- **Resolved dataset invariant**: `ResolvedTenant.datasets` always contains the tenant's `default_dataset`, whether or not a `datasets` row names it (`catalog_manager.rs::ensure_default_dataset`, applied by both the config and database descriptors). A dataset can be the default without having a row — admin-API tenant creation writes the name as a column on the tenant row only, and a config tenant may declare `default_dataset` with no `[[auth.tenants.datasets]]` block. Do not re-add this fallback at consumers: compaction planning, retention, orphan cleanup, and table reconciliation all iterate `.datasets` and get it for free (#1066)
