...
- Every catalog connection gets `journal_mode = wal`, `busy_timeout = 30000` and `synchronous = normal` as session statements (`sqlite_session_statements` in `iceberg/mod.rs`), so concurrent trace/log commits don't serialize behind a rollback lock and stall first-time table creation, and a commit contending with a compaction doesn't give up after sqlx's 5s default. The pool is lazy, so they land on first use. Needs the catalog's session-statement support (upstream JanKaul/iceberg-rust#386, currently a fork pin — see `Cargo.toml`)
